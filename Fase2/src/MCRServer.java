	...
	public static void main(String args[]) throws Exception {
		...
		if (args.length == 2 && (args[1].indexOf(':') >= 0)) {
			try {
				...
				SSOhost = args[0].substring(0, idx);
				SSOport = Integer.parseInt(args[0].substring(idx + 1));
			} catch (NumberFormatException e) {
				System.err.println("I will use port 9999, SSOHost = localhost, SSOPort = 8080");
			}
		}
		
		File serverConfig = new File("serverConfigs.txt");
		SecureTCPSocket serverCom = new SecureTCPSocket("localhost", 8080,new Config(serverConfig));
		
		serverCom.write("localhost_" + 8080, (byte)12);


		System.out.println("MCRServer started, PORT= " + port);
